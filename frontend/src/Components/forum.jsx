...
function Forum() {
  ...
  useEffect(() => {
    fetch(`${process.env.REACT_APP_BACKEND_URL}/api/discussions/${courseId}`)
      .then((res) => res.json())
      .then((data) => setMessage(data));
      fetch(`${process.env.REACT_APP_BACKEND_URL}/api/courses/${courseId}`).then((res)=>res.json()).then((data)=>setCourse(data));
  }, []);
  ...
  const addTask = () => {
    if (taskRef.current && taskRef.current.value.trim() !== "") {
      ...
      fetch(`${process.env.REACT_APP_BACKEND_URL}/api/discussions/addMessage`, {
        method: 'POST',
        headers: {
